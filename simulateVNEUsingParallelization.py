...
def timeSteps(locationAmount, individualAmount, spatialPreferenceVector, groupPreferenceVector,
              indEachGroup, rhoValue, time_stamps):
    largeTripartiteList = []
    truth = False
    if truth == False:
        IxS = []
...
#####################################################################################
# This is our 'loop' function to allow for parallelization
#####################################################################################
def loops(dumdum):    
    
    # Defining what specific we want in our simulations
    location_amount = 10
    ...
    # lists to collect all simulations and combinations of gamma, beta, and rho
    ...
    for z in range(len(gamma_values)):
        ...
        for k in range(len(beta_values)):
            ...
            for y in range(len(rho_values)):
                ...
                # This function forces each simulation to give us non-zero VNE values 
                # and also avoid VNE values that are infinity
                rhoVNEIxS, rhoVNEIxI, rhoVNETripartite, rhoVNEHomerange, rhoVNESxS = timeSteps(location_amount, individual_amount, spatial_preference_vector,
                 ...
                 # Collecting VNE values for a given combination of our 3 parameters
            ...
            # collects VNE values for all values of rho for every beta value given a
        ...
        # Collects the VNE for every Gamma, Beta, Rho combination 
    ...
    return(gamma_collection_VNE_IxS, 
           gamma_collection_VNE_IxI,
           gamma_collection_VNE_tripartite,
           gamma_collection_VNE_homerange_IxI,
           gamma_collection_VNE_SxS)
...
#####################################################################################
# This is our 'main' function allowing for paralellization
#####################################################################################
eachSimulation = []
...
if __name__ == '__main__':
    ...
    iterator = np.ones(simulation_amount)
    eachSimulation = pool.map_async(loops, iterator).get()
    ...
    IxSCollection = []
    IxICollection = []
    TripartiteCollection = []
    HomerangeCollection = []
    SxSCollection = []
    for i in range(simulation_amount):
        IxSCollection.append(eachSimulation[i][0])
        IxICollection.append(eachSimulation[i][1])
        TripartiteCollection.append(eachSimulation[i][2])
        HomerangeCollection.append(eachSimulation[i][3])
        SxSCollection.append(eachSimulation[i][4])
        
    np.save('IxSVNE10By10', IxSCollection)
    np.save('IxIVNE10By10', IxICollection)
    np.save('TripartiteVNE10By10', TripartiteCollection)
    np.save('HomerangeVNE10By10', HomerangeCollection)
    np.save('SxSVNE10By10', SxSCollection)
